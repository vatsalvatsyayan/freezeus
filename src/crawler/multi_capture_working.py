...
BASE_OUT = Path("out"); BASE_OUT.mkdir(exist_ok=True, parents=True)
...
() => {
  ...
  function scoreContainer(el) {
    ...
    score -= linkDensity * 2;
    score -= looksNav ? 2 : 0;
    return { score, textLen, linkDensity, hcount, isMain, looksNav };
  }
  ...
  return {
    ...
    meta: { kept_count: kept.length, total_candidates: scored.length, url: location.href, title: document.title }
  };
}
"""
...
def site_dir(domain: str) -> Path:
    p = BASE_OUT / domain; p.mkdir(exist_ok=True, parents=True); return p
...
def ensure_type_dirs(domain: str) -> Dict[str, Path]:
    ...
    for k, sub in TYPE_DIRS.items():
        d = root / sub; d.mkdir(parents=True, exist_ok=True); out[k] = d
...
def write_manifest(domain: str, seed_base: str, entries: List[dict], mode: str, stop_reason: str, cfg: dict):
    root = site_dir(domain)
    for e in entries:
        if "files" in e: e["files"] = {k: str(v) for k,v in e["files"].items()}
...
def write_outputs(domain: str, url: str, full_html: str, red_focus: str, red_lite: str, signals: list, meta: dict, page_id: str) -> Dict[str, Path]:
    ...
    meta = dict(meta or {}); meta["sha1"] = sha1(full_html or ""); meta["url"] = url; meta["page_id"] = page_id
...
async def domain_context(pw, domain: str, headed: bool):
    ...
    async def _route(route):
        if route.request.resource_type in BLOCK_RESOURCE_TYPES: return await route.abort()
    ...
    try:
        ...
        try:
            state = await context.storage_state(); storage_file.write_text(json.dumps(state), encoding="utf-8")
        except Exception:
            pass
...
async def snapshot_current(page: Page) -> Tuple[str, str, str, list, dict]:
    ...
    red_focus = focus.get("reduced_html",""); signals = focus.get("kept_signals",[]); meta_reduce = focus.get("meta",{})
    red_lite = await page.evaluate(REDUCE_LITE_JS)
    meta = {"url": page.url, "ts": int(time.time()), "title": meta_reduce.get("title"), "reduce_meta": meta_reduce}
    ...
    for attempt in range(1, MAX_RETRIES+1):
        try:
            ...
            if code in (403,429,503):
                ...
                print(f"[backoff] {code}→sleep {backoff:.1f}s"); await page.wait_for_timeout(int(backoff*1000)); continue
            break
        except Exception as e:
            meta["error"] = str(e)[:300]
            backoff = (2**attempt)+random.random()*2
            print(f"[error] {e}→sleep {backoff:.1f}s"); await page.wait_for_timeout(int(backoff*1000))
...
async def try_click_load_more(page: Page) -> bool:
    ...
    try:
        loc = page.locator(sel); count = await loc.count()
        for i in range(min(count, 140)):
            ...
            if any(re.search(p, txt, re.I) for p in patterns):
                if await el.is_enabled() and await el.is_visible():
                    await el.click(); print(f"[click] load-more: {txt[:60]}")
...
async def crawl_seed(page: Page, seed_url: str,
    ...
    start = time.time()
    clicks = 0; scrolls = 0; no_change_rounds = 0
    grew_any = False; stop_reason = "none"
    ...
    while True:
        if len(await ordered_job_hrefs(page, seed_url)) >= jobs_max:
            stop_reason = "jobs_cap"; break
        if (time.time() - start) >= time_budget:
            stop_reason = "time"; break
        ...
        if progressed_something: continue
        if no_change_rounds >= no_change_cap:
            stop_reason = "no_change"; break
    ...
    while pages_seen < pages_max:
        ...
        for _ in range(20):
            ...
            if ok: break
        if not ok:
            stop_reason = "no_change"; print("[paginate] next produced no change"); break

        full_html, red_focus, red_lite, signals, meta = await snapshot_current(page)
        pages_seen += 1; pid = f"p{pages_seen:03d}"
    ...
    write_manifest(domain, seed_base, entries, mode="pagination",
...
async def crawl(urls: List[str], headed: bool,
    ...
    for u in urls: by_domain.setdefault(domain_of(u), []).append(u)
...
if __name__ == "__main__":
    ...
    if not url_file.exists():
        print(f"[error] URLs file not found: {url_file}", file=sys.stderr); sys.exit(1)

    targets = read_urls_from_file(url_file)
    if not targets:
        print("[error] No URLs found in file.", file=sys.stderr); sys.exit(1)
